 ...
 */
var Character = function(game, position, properties) {
	Phaser.Sprite.call(
		this,
		game,
		position.x,
		position.y,
		properties.spriteName);
...
Character.prototype.giveMeName = function () {
	return (
		this.clothingType.head.name +
		this.clothingType.body.name +
		'Bot'
	);
};
...
Character.prototype.showAura = function (callback, elem) {
	var auraTween = this.game.add.tween(this.emitterGroup.aura).to(
		{frequency: 10}, 3500, Phaser.Easing.Linear.None, true);
...
Character.prototype.updateClothing = function (bodyPartName, clothingType) {
	var firstHeadFrameCurrentType, firstBodyFrameCurrentType,
		self = this;
	...
	function updateAnimationFrames(bodyPart, type, animationsInfo) {
		...
		for (var currentAnimation in animationManager._anims) {
			if (animationsInfo.hasOwnProperty(currentAnimation)){
				//Copy frames array
				var auxFramesData =
					(animationsInfo[currentAnimation].frameSecuence).slice();
	...
	case 'body':
		...
		firstBodyFrameCurrentType =
			this.firstBodySpritePositionArray[clothingType];
	...
	case 'head':
		...
		firstHeadFrameCurrentType =
			this.firstHeadSpritePositionArray[clothingType];
...
Character.prototype.restartAnimation = function (name) {
	var firstBodyFrameCurrentType =
			this.firstBodySpritePositionArray[this.clothingType.body.type];
