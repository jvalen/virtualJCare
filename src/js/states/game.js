  ...
  Game.prototype = {
        ...
        if (this.game.world.bounds.height === this.game.height) {
            this.newWorldBoundaries = {
                ...
                height: (this.game.world.bounds.height)-
                        (this.game.world.bounds.height / 4)
            };

            //Reduce world y axis boundaries
            this.game.world.setBounds(
                0, 0, this.newWorldBoundaries.width,
                    ...
                    var self = this,
                        callback = function(e){
                            ...
                            self.rockIntervalTime -= (
                                self.rockIntervalTime /
                                self.confData.rockFrecuencyDivisor);
    ...
    update: function () {
        ...
        this.itemsGroup.forEachAlive(function(item){
            this.game.physics.arcade.overlap(this.character, item, function(){
     ...
     * @param {object} event
     */
    throwItem: function(spriteType, currentProperties, event){
    ...
    throwRock: function(){
        ...
        positionX = Math.random() < 0.5 ?
            (this.ITEM_WIDTH) : (this.newWorldBoundaries.width - this.ITEM_WIDTH + 1),
     ...
     */
    showStatsUpdateInfo: function(position, amount, positive) {
    ...
    createUi: function () {
        ...
        for (var i = 0; i < buttonsCollection.length; i++) {
            ...
            if(buttonsCollection[i].order === 1) {
                ...
                buttonsCollection[i].order ===
                buttonsCollection[i].length
                ) {
                currentButton['x'] =
                    this.game.width - (screenButtonSize / 2);
            } else {
                currentButton['x'] =
                    screenButtonSize *
                    buttonsCollection[i].order -
        ...
        for (var i = 0; i < buttonArrayInfo.length; i++) {
            ...
            this[currentItem.name] = this.game.add.sprite(
                currentItem.x,
                currentItem.y, currentItem.spritesheet);
        ...
        this.healthLabel = this.game.add.bitmapText(
            position1, 70, 'minecraftia',
            this.confData.text[this.locale].health, 16);
        ...
        this.funLabel = this.game.add.bitmapText(
            position2, 70, 'minecraftia',
            this.confData.text[this.locale].fun, 16);
        this.funLabel.position.x =
            this.funLabel.position.x - this.funLabel.textWidth / 2;

        this.nutritionLabel = this.game.add.bitmapText(
            position3, 70, 'minecraftia',
            this.confData.text[this.locale].nutrition, 16);
        ...
        this.healthCounter = this.game.add.bitmapText(
            position1, 100, 'minecraftia', '00', 35);
        ...
        this.funCounter = this.game.add.bitmapText(
            position2, 100, 'minecraftia', '00', 35);
        ...
        this.nutritionCounter = this.game.add.bitmapText(
            position3, 100, 'minecraftia', '00', 35);
        this.nutritionCounter.position.x =
            this.nutritionCounter.position.x -
            this.nutritionCounter.textWidth / 2;
     ...
     * @param {object} event
     */
    selectOption: function (sprite, event) {
        if(!this.uiBlocked) {
            this.uiBlocked = true;
            ...
            case 2:
              this.throwItem(
                  sprite.type,
                  this.selectedItem.customParams,
                  event);
              break;
            case 3:
              this.character.animations.play('jump');
              this.character.body.velocity.y =
                this.confData.physics.jump.velocity.y;
              this.jumping = true;
     ...
     */
    refreshStats: function(properties) {
        if (!!properties) {
        ...
        if (this.character.customParams.health <= 0 ||
            ...
            messages.tryAgain =
                this.confData.text[this.locale].tryAgain.toUpperCase();
        ...
        //Update stats
        this.healthCounter.setText(
            this.character.customParams.health.toString());
        this.funCounter.setText(
            this.character.customParams.fun.toString());
     ...
     */
    walkTo: function(sprite, pointer, targetPosition) {
    ...
    gameOver: function(messages){
        if (!this.gameOverFlag){
            ...
            gameOverPanelTween = this.game.add.tween(this.gameOverPanel).to(
                {alpha: 0.8}, 300, Phaser.Easing.Linear.None, true);
            ...
            gameOverPanelTween.onComplete.add(function(){
                var highscoreData = getLocalItem('highscore'),
                    ...
                    gameOverText, reasonText, scoreLabel, scoreCounter, creditsText,
                    topLabel, topCounter, retryButton, tweetButton, fbButton,
                    clothCombinationText, clothCombinationLabel,
                    ...
                    sharedHeadSprite, sharedBodySprite;
                ...
                this.game.add.tween(gameOverItemsGroup.position).to(
                    {y: 0},1000, Phaser.Easing.Back.InOut, true);
            }, this);
    ...
    tweetScore: function(score, name){
        //Tweet score
    ...
    fbScore: function(score, name){
        ...
        var fbUrl = 'https://www.facebook.com/dialog/feed?',
            ...
            fbParams =
                ...
                'description=' + message + '&' +
                'link=' + url + '&' +
                'redirect_uri=' + this.confData.url,
